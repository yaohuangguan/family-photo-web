...
function solarTerm(date){
  const y=date.getFullYear(),key=dateKey(date);
  if(y<1900||y>2100)return "";
  for(let i=0;i<24;i++){
    const ms=Date.UTC(1900,0,6,2,5)+31556925974.7*(y-1900)+termInfo[i]*60000;
    const d=new Date(ms);
    if(`${y}-${pad(d.getUTCMonth()+1)}-${pad(d.getUTCDate())}`===key)return termNames[i];
  }
  return "";
}
function dayMeta(date){
  const lunar=lunarInfo(date),term=solarTerm(date);
  const solar=solarFestivals[`${date.getMonth()+1}-${date.getDate()}`]||"";
  let lunarFest=lunarFestivals[`${lunar.month}-${lunar.day}`]||"";
  const tomorrow=new Date(date);tomorrow.setDate(date.getDate()+1);
  const next=lunarInfo(tomorrow);if(next.month==="正月"&&next.day===1)lunarFest="除夕";
  const festival=solar||lunarFest;
  return {lunar,term,festival,label:festival||term||lunar.label};
...
function renderCalendar(){
  ...
  for(let i=0;i<42;i++){
    ...
    const dt=new Date(cy,cm,d),meta=dayMeta(dt);
    const isToday=dateKey(dt)===dateKey(today),isSelected=dateKey(dt)===dateKey(cal.selected);
...
function renderSelectedDay(){
  const d=cal.selected,m=dayMeta(d);
  const week=["日","一","二","三","四","五","六"][d.getDay()];
  const bits=[`农历${m.lunar.month}${m.lunar.day===1?"初一":m.lunar.label}`,m.term,m.festival].filter(Boolean);
...
qs("#calendar-grid").onclick=e=>{
  ...
  cal.selected=new Date(y,m-1,d);
  if(cal.view.getMonth()!==m-1||cal.view.getFullYear()!==y)cal.view=new Date(y,m-1,1);
  renderCalendar();loadWeather(cal.selected);
};
renderCalendar();function updateLocalTimes(){
  for(const [name,tz] of [["shijiazhuang","Asia/Shanghai"],["auckland","Pacific/Auckland"]]){
...
async function loadWeather(date){
  const req=++weatherRequest,key=dateKey(date),today=dateKey(new Date());
  qsa(".weather-card").forEach(c=>{c.classList.add("loading");c.classList.remove("unavailable");});
  qs("#weather-date-title").textContent=key===today?"今天":`${date.getFullYear()}年${date.getMonth()+1}月${date.getDate()}日`;
  qs("#weather-date-note").textContent=key===today?"当前天气":"历史 / 预报";
  try{
    const res=await fetch(`/api/weather?date=${key}`,{cache:"no-store"});
    if(req!==weatherRequest)return;
    const data=await res.json();
    if(!res.ok)throw new Error(data?.error||"weather");
    for(const name of ["shijiazhuang","auckland"]){
      const card=qs(`[data-city="${name}"]`),w=data[name];card.classList.remove("loading");
      if(!w||w.unavailable){
        card.classList.add("unavailable");qs(".temperature",card).textContent="暂无";
        qs(".weather-desc",card).textContent=w?.reason||"暂无该日期天气数据";
        qs(".feels",card).textContent="";qs(".wind",card).textContent="";qs(".weather-icon",card).textContent="—";continue;
      }
      const [desc,icon]=weatherCode[w.weather_code]||["天气变化中","🌤️"];qs(".weather-icon",card).textContent=icon;
      if(w.mode==="current"){
        qs(".temperature",card).textContent=Math.round(w.temperature_2m);qs(".weather-desc",card).textContent=desc;
        qs(".feels",card).textContent=`体感 ${Math.round(w.apparent_temperature)}°`;qs(".wind",card).textContent=`风速 ${Math.round(w.wind_speed_10m)} km/h`;
      }else{
        qs(".temperature",card).textContent=`${Math.round(w.temperature_2m_min)}–${Math.round(w.temperature_2m_max)}`;
        qs(".weather-desc",card).textContent=desc;qs(".feels",card).textContent=`体感 ${Math.round(w.apparent_temperature_min)}–${Math.round(w.apparent_temperature_max)}°`;
        qs(".wind",card).textContent=`最大风速 ${Math.round(w.wind_speed_10m_max)} km/h`;
      }
    }
  }catch{
    if(req!==weatherRequest)return;
    qsa(".weather-card").forEach(card=>{card.classList.remove("loading");qs(".weather-desc",card).textContent="天气暂时获取失败";});
  }
}
...
function renderProfile(){
  qs("#profile-name").value=profile.name;qs("#profile-avatar").textContent=profile.avatar;
  qs("#avatar-picker").innerHTML=avatars.map(a=>`<button class="avatar-choice ${a===profile.avatar?"active":""}" data-avatar="${a}">${a}</button>`).join("");
}
...
function renderFamilyState(data){
  const visitors=data.visitors||[];qs("#visitor-count").textContent=`${visitors.length} 人`;
  qs("#visitor-list").innerHTML=visitors.length?visitors.map(v=>`<span class="visitor-chip"><span class="avatar">${v.avatar}</span>${escapeHtml(v.name)}</span>`).join(""):'<span class="message-empty">今天还没人留下脚印</span>';
...
function connectBoard(){
  clearTimeout(retryTimer);const proto=location.protocol==="https:"?"wss":"ws";socket=new WebSocket(`${proto}://${location.host}/api/board`);setStatus("","正在连接");
...
qs("#clear-btn").onclick=()=>{if(confirm("确定要清空全家的涂鸦板吗？这个操作会同步给所有人。"))send({type:"clear"});};
new ResizeObserver(resizeCanvas).observe(canvas);connectBoard();